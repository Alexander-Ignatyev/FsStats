...
[<Fact>]
let ``Difference of two means`` () =
    // Generated by Normal.Distribution(10.0, 2.0).Samples 30
    let s1 = create data
    // Fake Normal.Distribution(10.0, 2.0)
    let s2 = { Mean = 10.0; StdDev = 2.0; Size = 30; Data = Array.empty; Sorted = false }
    Two.stderr s1 s2 |> should (equalWithin 1e-5) 0.51911
    Two.marginOfError s1 s2 0.95 |> should (equalWithin 1e-5) 1.01696
    let l, r = Two.confidenceInterval s1 s2 0.95
    // confidence interval around 0, as expected
    l |> should (equalWithin 1e-5) -1.0299
    r |> should (equalWithin 1e-5) 1.00402
